...
class AgentInput:
    ...
    lidars: List[Lidar]
    map_name: str = "us-nv-las-vegas-strip"
    ...
    def from_scene_dict_list(
        ...
        for frame_idx in range(num_history_frames):
            ...
            # NOTE synthetic data
            lidar_path = scene_dict_list[frame_idx].get("lidar_path")
        ...
        map_name = scene_dict_list[num_history_frames - 1].get("map_location", "us-nv-las-vegas-strip")
        token = scene_dict_list[num_history_frames - 1]["token"]
        return AgentInput(ego_statuses, cameras, lidars, map_name=map_name, token=token, global_ego_pose=global_ego_poses[-1])
...
class Scene:
    ...
    def get_agent_input(self) -> AgentInput:
        ...
        map_name = self.scene_metadata.map_name
        
        # 2. Get the token (last frame of history = current frame)
        current_frame_idx = self.scene_metadata.num_history_frames - 1
        token = self.frames[current_frame_idx].token

        current_frame = self.frames[current_frame_idx]
        global_ego_pose = current_frame.ego_status.ego_pose
        return AgentInput(
            ...
            map_name=map_name,
            token=token,
            global_ego_pose=global_ego_pose
        )
