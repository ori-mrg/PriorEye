...
class CacheOnlyDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        logger.info(
            f"[CacheOnlyDataset] split={split}, total_tokens={len(self.tokens)}"
        )
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor], str]:
        """
        Loads and returns pair of feature and target dict from data AND the token.
        :param idx: index of sample to load.
        :return: tuple of (feature, target, token)
        """
    ...
    def _load_valid_caches(
        ...
        # -----------------------
        # 2. SimScale synthetic
        # -----------------------
        SYN_IDX = os.environ.get("SYN_IDX", None)
        if SYN_IDX is None:
            logger.info("SYN_IDX not set → using base cache only")
            return valid_cache_paths

        SYN_GT = os.environ.get("SYN_GT", None)
        assert SYN_GT is not None, "SYN_GT must be set when SYN_IDX is used"

        yaml_base = Path(os.environ["NAVSIM_DEVKIT_ROOT"]) / "navsim/planning/script/config/training"
        cache_base = Path(os.environ["NAVSIM_DEVKIT_ROOT"]) / "cache"

        syn_indices = [int(x.strip()) for x in SYN_IDX.split(",")]
        ...
        for idx in syn_indices:
            ...
            with open(yaml_file, "r") as f:
                syn_log_names = yaml.safe_load(f)[f"{split}_logs"]

            agent_name = os.environ.get("AGENT", "")
            prefix = f"{agent_name}_" if agent_name else ""
            syn_cache_path = cache_base / f"{prefix}synthetic_reaction_{SYN_GT}_v1.0-{idx}_cache"

            for log_name in tqdm(syn_log_names, desc=f"[SimScale] {SYN_GT} v1.0-{idx} ({split})"):
                log_path = syn_cache_path / log_name
                ...
                for token_path in log_path.iterdir():
                    found = True
                    for builder in feature_builders + target_builders:
                        if not (token_path / (builder.get_unique_name() + ".gz")).is_file():
                            found = False
                            break
                    if found:
                        valid_cache_paths[token_path.name] = token_path
            ...
            logger.info(
                            f"[SimScale] loading round {idx}, split={split}, num_logs={len(syn_log_names)}"
                        )
            logger.info(
                f"[SimScale] after round {idx}, total_tokens={len(valid_cache_paths)}"
            )
        return valid_cache_paths

    def _load_scene_with_token(self, token: str) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor], str]:
        """
        Helper method to load sample tensors given token
        :param token: unique string identifier of sample
        :return: tuple of feature and target dictionaries and token
        """
        ...
        # [Modified] Return the token along with features and targets.
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor], str]:
        ...
        :param idx: index of sample to load.
        :return: tuple of feature and target dictionary and token
        """
