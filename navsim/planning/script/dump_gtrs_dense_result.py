...
plt.rcParams.update(
    {
        ...
        "font.size": 9,  # Body text is 10pt, so 8-9pt is appropriate
        "axes.titlesize": 10,  # Title is slightly larger
...
def main(cfg: DictConfig) -> None:
    ...
    for batch in predictions:
        for token, data in batch.items():
            results_map[token] = data

    pickle.dump(results_map, open(OUTPUT_PATH, "wb"))
    print(f"Subscore/Trajectories saved to {OUTPUT_PATH}")
