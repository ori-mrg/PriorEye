...
from navsim.planning.script.run_pdm_score_one_stage import (
    ...
    create_scene_aggregators as create_scene_aggregators_one_stage,
    compute_final_scores as compute_final_scores_one_stage,
)
...
def main(cfg: DictConfig) -> None:
    ...
    try:
        ...
        scored_tokens = set(pdm_score_df["token"].values)
        ...
        for orig_token, prev_token, two_stage_pairs in raw_mapping:
            if prev_token in scored_tokens or orig_token in scored_tokens:
                valid_pairs = [
                    ...
                    if pair[0] in scored_tokens and pair[1] in scored_tokens
                ]
                if valid_pairs:
                    all_mappings[(orig_token, prev_token)] = valid_pairs

        if not all_mappings:
            raise ValueError("No valid two-stage mappings found (synthetic scenes may be excluded)")
    ...
    if pseudo_closed_loop_valid and all_mappings:
        ...
        pcl_group_score = pd.Series(dtype=float)
        pcl_stage1_score = pd.Series(dtype=float)
        pcl_stage2_score = pd.Series(dtype=float)
    ...
    if pseudo_closed_loop_valid and all_mappings:
        ...
        # Stage 1 only: compute two_frame_extended_comfort using reactive_all_mapping first-stage pairs
        # This ensures same comfort coverage as the two-stage path
        raw_mapping = cfg.train_test_split.reactive_all_mapping
        scored_tokens = set(pdm_score_df["token"].values)
        ...
        pdm_score_df["weight"] = 1.0
        pdm_score_df = pdm_score_df.set_index("token")

        for orig_token, prev_token, _ in raw_mapping:
            if orig_token in scored_tokens and prev_token in scored_tokens:
                aggregator = SceneAggregator(
                    now_frame=orig_token,
                    previous_frame=prev_token,
                    score_df=pdm_score_df,
                    proposal_sampling=proposal_sampling,
                )
                comfort = aggregator._compute_two_frame_comfort(orig_token, prev_token)
                pdm_score_df.loc[orig_token, "two_frame_extended_comfort"] = comfort
                pdm_score_df.loc[prev_token, "two_frame_extended_comfort"] = comfort

        pdm_score_df.reset_index(inplace=True)
        pdm_score_df = pdm_score_df.drop(columns=["ego_simulated_states"])
        pdm_score_df = compute_final_scores_one_stage(pdm_score_df)

        # Recompute score_cols now that score and two_frame_extended_comfort exist
        score_cols = [
            c
            for c in pdm_score_df.columns
            if (
        ...
        # Split into stage_one columns (all frames are original)
        for col in score_cols:
            pdm_score_df[f"{col}_stage_one"] = pdm_score_df[col]

        pdm_score_df.drop(columns=score_cols, inplace=True)
        # Recreate score from score_stage_one (same pattern as two-stage path)
        pdm_score_df["score"] = pdm_score_df["score_stage_one"]
        pdm_score_df.drop(columns=["score_stage_one"], inplace=True)

        stage1_cols = [f"{col}_stage_one" for col in score_cols if col != "score"]
        score_cols = stage1_cols + ["score"]

        pdm_score_df = pdm_score_df[["token", "valid"] + score_cols]

        summary_rows = []
        valid_df = pdm_score_df[pdm_score_df["valid"] == True]

        stage1_row = pd.Series(index=pdm_score_df.columns, dtype=object)
        stage1_row["token"] = "extended_pdm_score_stage_one"
        stage1_row["valid"] = True
        for col in score_cols:
            stage1_row[col] = valid_df[col].mean() if col in valid_df.columns else np.nan
        summary_rows.append(stage1_row)
        ...
        for col in score_cols:
            combined_row[col] = valid_df[col].mean() if col in valid_df.columns else np.nan
        summary_rows.append(combined_row)
