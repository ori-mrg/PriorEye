...
from navsim.agents.memory.memory_feature_builder import MemoryFeatureBuilder
import torch
...
def state2traj(states):
    rel_poses = absolute_to_relative_poses([StateSE2(*tmp) for tmp in states[:, StateIndex.STATE_SE2]])
    final_traj = [pose.serialize() for pose in rel_poses[1:]]
    return final_traj


def apply_fixed_mud_splash(image):
    ...
    for _ in range(12):
        rand_x = center_x + np.random.randint(-100, 100)
        rand_y = center_y + np.random.randint(-60, 60)
        axes = (np.random.randint(80, 150), np.random.randint(40, 100))
        angle = np.random.randint(0, 180)
        cv2.ellipse(image, (rand_x, rand_y), axes, angle, 0, 360, mud_color, -1)

    # 2. Particle muds
    for _ in range(100):
        sx, sy = np.random.randint(0, w), np.random.randint(0, h)
        cv2.circle(image, (sx, sy), np.random.randint(1, 30), mud_color, -1)
...
def apply_blackout(image):
    image.fill(0)  # black
    return image


def apply_low_visibility_fog(image):
    """
    Dense fog effect: combines strong blur and brightness increase to remove visual features.
    """
    h, w = image.shape[:2]
    # 1. Very strong blur (removes lane lines and object contours)
    image = cv2.GaussianBlur(image, (51, 51), 0)

    # 2. Overlay a light gray filter for a fog-like effect
    fog_overlay = np.full((h, w, 3), 200, dtype=np.uint8)  # light gray
    # Blend 40% original + 60% fog
    image = cv2.addWeighted(image, 0.4, fog_overlay, 0.6, 0)

    return image


def visualize_camera_fault(l0_img, f0_img, r0_img, fig_file_name):
    # stiched = np.concatenate([l0_img, f0_img, r0_img], axis=1)
    stiched = np.concatenate([f0_img], axis=1)
    fig, axes = plt.subplots(1, 1, figsize=(7, 5))
    axes.imshow(stiched)
    axes.axis("off")
    plt.tight_layout()
    save_path = f"./fault_images/{fig_file_name}.png"
...
class HydraFeatureBuilder(AbstractFeatureBuilder):
    def __init__(self, config: HydraConfig):
        ...
        self._memory = MemoryFeatureBuilder(
            memory_embedding_model=config.memory_embedding_model,
            memory_corruption_test=config.memory_corruption_test,
            visualize=config.visualize,
        )

        self._mud_splash = False
    ...
    def compute_features(self, agent_input: AgentInput) -> Dict[str, torch.Tensor]:
        ...
        self._memory.compute_to_dict(agent_input, features)

        # 1. Front Camera
        raw_cam_front = self._get_camera_feature(agent_input)
        # Save as uint8 to reduce storage
        features["camera_feature"] = [(f * 255).to(torch.uint8) if f is not None else None for f in raw_cam_front]
        # 2. Back Camera
        raw_cam_back = self._get_camera_feature_back_multiple(agent_input)
        features["camera_feature_back"] = [(f * 255).to(torch.uint8) if f is not None else None for f in raw_cam_back]
        # 3. Left/Right Cameras
        if self._config.use_lr_view:
            raw_cam_left = self._get_camera_feature_single(agent_input, "left")
            features["camera_feature_left"] = [
                (f * 255).to(torch.uint8) if f is not None else None for f in raw_cam_left
            ]

            raw_cam_right = self._get_camera_feature_single(agent_input, "right")
            features["camera_feature_right"] = [
                (f * 255).to(torch.uint8) if f is not None else None for f in raw_cam_right
            ]
    ...
    def _get_camera_feature(self, agent_input: AgentInput) -> torch.Tensor:
        ...
        for idx, camera in enumerate(cameras):
            ...
            if image is not None and image.size > 0 and np.any(image):
                ...
                if self._mud_splash:
                    # visualize_camera_fault(l0, f0, r0, f"{unique_id}_before")
                    l0 = apply_fixed_mud_splash(l0)
                    f0 = apply_fixed_mud_splash(f0)
                    r0 = apply_fixed_mud_splash(r0)
                    # visualize_camera_fault(l0, f0, r0, f"{unique_id}_after")
                if self._fog:
                    # visualize_camera_fault(l0, f0, r0, f"{unique_id}_fog_before")
                    l0 = apply_low_visibility_fog(l0)
                    f0 = apply_low_visibility_fog(f0)
                    r0 = apply_low_visibility_fog(r0)
                    # visualize_camera_fault(l0, f0, r0, f"{unique_id}_fog_after")

                if self._blackout:
                    # visualize_camera_fault(l0, f0, r0, f"{unique_id}_{idx}_black_before")
                    l0 = apply_blackout(l0)
                    f0 = apply_blackout(f0)
                    r0 = apply_blackout(r0)
                    # visualize_camera_fault(l0, f0, r0, f"{unique_id}_{idx}_black_after")
        ...
        for camera in cameras:
            ...
            if image is not None and image.size > 0 and np.any(image):
                ...
                resized_image = cv2.resize(
                    single_img, (int(self._config.camera_width * 1920 / 4096), self._config.camera_height)
                )
                tensor_image = transforms.ToTensor()(resized_image)
                image_list.append(tensor_image)
            else:
...
class HydraTargetBuilder(AbstractTargetBuilder):
    def __init__(self, config: HydraConfig):
        ...
        self.simulator = PDMSimulator(TrajectorySampling(num_poses=40, interval_length=0.1))
    ...
    def compute_targets(self, scene: Scene) -> Dict[str, torch.Tensor]:
        """Inherited, see superclass."""
        future_traj = scene.get_future_trajectory(num_trajectory_frames=int(4 // 0.5))
        ...
        ego_state = EgoState.build_from_rear_axle(
            ...
            rear_axle_velocity_2d=StateVector2D(*scene.frames[frame_idx].ego_status.ego_velocity),
            rear_axle_acceleration_2d=StateVector2D(*scene.frames[frame_idx].ego_status.ego_acceleration),
        )
