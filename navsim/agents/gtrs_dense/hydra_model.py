...
class HydraModel(nn.Module):
    ...
    def _restore_uint8_to_float(self, feat: Union[torch.Tensor, List[torch.Tensor]]):
        if isinstance(feat, list):
            # If it is a list, recursively convert each element
            return [self._restore_uint8_to_float(f) for f in feat]
        elif isinstance(feat, torch.Tensor) and feat.dtype == torch.uint8:
            # If tensor with uint8 dtype, convert to float
            return feat.float() / 255.0
        return feat

    def evaluate_dp_proposals(self, features, dp_proposals, topk=10, dp_only_inference=False):
        # Convert images from uint8 to float
        features["camera_feature"] = self._restore_uint8_to_float(features["camera_feature"])
        features["camera_feature_back"] = self._restore_uint8_to_float(features["camera_feature_back"])

        if "camera_feature_left" in features and features["camera_feature_left"] is not None:
            features["camera_feature_left"] = self._restore_uint8_to_float(features["camera_feature_left"])

        if "camera_feature_right" in features and features["camera_feature_right"] is not None:
            features["camera_feature_right"] = self._restore_uint8_to_float(features["camera_feature_right"])
        ...
        if self.use_memory:
            ...
            status_q = status_encoding.unsqueeze(1)  # [B, 1, 256]
            combined_query = torch.cat([status_q, keyval], dim=1)  # [B, 1025, 256]
            ...
            status_encoding = fused_out[:, 0]  # [B, 256]
            keyval = fused_out[:, 1:]  # [B, 1024, 256]
        # =================================================================

        output: Dict[str, torch.Tensor] = {}
        # =================================================================
        # FP 32 for trajectory head
        # =================================================================
        with autocast(enabled=False):
            # Explicitly convert inputs to float32
            keyval_32 = keyval.float()
            status_encoding_32 = status_encoding.float()
            dp_proposals_32 = dp_proposals.float()

            trajectory = self._trajectory_head.eval_dp_proposals(
                keyval_32, status_encoding_32, dp_proposals_32, topk=topk, dp_only_inference=dp_only_inference
            )
        # =================================================================
    ...
    def forward(self, features: Dict[str, torch.Tensor], interpolated_traj=None) -> Dict[str, torch.Tensor]:
        ...
        status_feature: torch.Tensor = features["status_feature"][0]  # [B, 8]
        camera_feature = features["camera_feature"]
        ...
        keyval += self._keyval_embedding.weight[None, ...]  # [B, 1024, 256]
        ...
        with autocast(enabled=False):
            ...
            interpolated_traj_32 = None
            if interpolated_traj is not None:
                interpolated_traj_32 = interpolated_traj.float()

            trajectory = self._trajectory_head(keyval_32, status_encoding_32, interpolated_traj_32)
        # =================================================================
...
class HydraTrajHead(nn.Module):
    def __init__(
        ...
        self.transformer = nn.TransformerDecoder(
            nn.TransformerDecoderLayer(d_model, nhead, d_ffn, dropout=0.0, batch_first=True), nlayers
        )
        self.vocab = nn.Parameter(torch.from_numpy(np.load(vocab_path)), requires_grad=False)
        ...
        if self.normalize_vocab_pos:
            self.encoder = MemoryEffTransformer(d_model=d_model, nhead=nhead, dim_feedforward=d_model * 4, dropout=0.0)
    ...
    def eval_dp_proposals(
        self, bev_feature, status_encoding, dp_proposals, topk=10, dp_only_inference=False
