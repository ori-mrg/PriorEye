...
class TransfuserModel(nn.Module):
    ...
    def __init__(self, trajectory_sampling: TrajectorySampling, config: TransfuserConfig):
        ...
        if self.use_memory:
            ...
            self._memory_module = MemoryAugmentationModule(
                embed_dim=config.tf_d_model,
                memory_in_dim=self.memory_embedding_dim,
            )
            # LoRA
            if config.use_lora:
                lora_cfg = LoraConfig(
        ...
        if config.use_lora:
            for name, param in self.named_parameters():
                ...
                elif "_memory_module" in name:  # memory module needs full fine tuning
                    param.requires_grad = True
                elif any(x in name for x in ["_agent_head", "_trajectory_head", "_bev_semantic_head", "_tf_decoder"]):
                    param.requires_grad = True
                else:
    ...
    def forward(self, features: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        status_encoding = self._status_encoding(status_feature)  # [64, 256]

        keyval = torch.concatenate([bev_feature, status_encoding[:, None]], dim=1)
        keyval += self._keyval_embedding.weight[None, ...]  # [64, 65, 256]
        ...
        if self.use_memory:
            ...
            keyval, memory_info = self._memory_module(
                current_states=keyval, memory_embedding=memory_embedding, memory_pos=memory_pos, return_info=True
            )

        query = self._query_embedding.weight[None, ...].repeat(batch_size, 1, 1)  # [64, 31, 256]
