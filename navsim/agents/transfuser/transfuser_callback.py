...
class TransfuserCallback(pl.Callback):
    ...
    def on_validation_epoch_end(self, trainer: pl.Trainer, lightning_module: pl.LightningModule) -> None:
        ...
        for idx_plot in range(self._num_plots):
            ...
            self._add_image_to_logger(trainer, "val", idx_plot, grid)
    ...
    def on_train_epoch_end(
        ...
        for idx_plot in range(self._num_plots):
            ...
            grid = self._visualize_model(features, targets, predictions)
            self._add_image_to_logger(trainer, "train", idx_plot, grid)

    def _add_image_to_logger(self, trainer: pl.Trainer, train_val: str, idx_plot: int, grid: torch.Tensor):
        if isinstance(trainer.logger, WandbLogger):
            # WandB 
            trainer.logger.experiment.log({
                f"{train_val}_plot_{idx_plot}": [wandb.Image(grid, caption=f"{train_val} Plot {idx_plot}")]
            })
        else:
            trainer.logger.experiment.add_image(f"{train_val}_plot_{idx_plot}", grid, global_step=trainer.current_epoch)
    ...
    def _visualize_model(
        ...
        if "lidar_feature" in features:
            ...
            # latent mode: no lidar input, use blank background for the overlay panel
            batch_size = camera.shape[0]
            lidar_map = np.zeros(
                (batch_size, self._config.lidar_resolution_height, self._config.lidar_resolution_width),
                dtype=np.float32,
            )
...
def dict_to_device(data: Dict[str, Any], device: Union[torch.device, str]) -> Dict[str, Any]:
    """
    Helper function to move tensors from dictionary to device.
    Recursively handles nested dictionaries and ignores non-tensor values.
    """
    # Create a new dictionary to avoid modifying the original data (optional but safer)
    new_data = {}
