...
def attention(q, k, v, mask=None, causal=False, attn_bias=None, **kwargs):
    scale = q.shape[-1] ** -0.5
    q = q * scale
...
def summarize_qkv_chunk(q, k, v, mask, attn_bias_chunk, causal, qk_start_indices, dropout):
    q_start_index, k_start_index, q_chunk_size, k_chunk_size, device = (
        *qk_start_indices,
        q.shape[-2],
        k.shape[-2],
        q.device,
...
class MemoryEffTransformer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation=F.relu, layer_norm_eps=1e-5):
        ...
        self.self_attn = Attention(dim=d_model, heads=nhead, dim_head=dim_head, memory_efficient=True)
    ...
    def forward(self, x, need_mean=False):
        ...
        if need_mean:
            num_query, embed_dims, bs, num_bev_queue = (q.shape[1], q.shape[2], q.shape[0] // 2, 2)
