...
class DrivoRFeatureBuilder(AbstractFeatureBuilder):
    def __init__(self, config: Dict):
        ...
        if isinstance(config, dict):
            mem_model = config.get("memory_embedding_model", "SIGLIP2")
            mem_corrupt = config.get("memory_corruption_test", False)
            mem_vis = config.get("visualize", False)
        else:
            mem_model = getattr(config, "memory_embedding_model", "SIGLIP2")
            mem_corrupt = getattr(config, "memory_corruption_test", False)
            mem_vis = getattr(config, "visualize", False)

        self._memory = MemoryFeatureBuilder(
            memory_embedding_model=mem_model,
            memory_corruption_test=mem_corrupt,
            visualize=mem_vis,
        )
    ...
    def _get_camera_feature(self, agent_input: AgentInput) -> torch.Tensor:
        ...
        data = {"image": torch.stack(images), "cam_K": torch.stack(cam_Ks), "world_2_cam": torch.stack(lidar2cams)}
