...
class timm_ViT(VisionTransformer):
    ...
    def forward_features(
        self, x: torch.Tensor, scene_tokens: torch.Tensor = None, attn_mask: torch.Tensor = None
    ) -> torch.Tensor:
...
class ImgEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.use_feature_pooling:
            self.pool_proj = torch.nn.Sequential(torch.nn.AdaptiveAvgPool1d(self.num_prefix_tokens))
