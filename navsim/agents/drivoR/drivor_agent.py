...
class LitProgressBar(ProgressBar):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        if batch_idx % 100 == 0:
            print(
                f"Epoch {trainer.current_epoch} - train {batch_idx} / {self.total_train_batches} - {self.get_metrics(trainer, pl_module)}"
            )
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        if batch_idx % 100 == 0:
            print(
                f"Epoch {trainer.current_epoch} - val {batch_idx} / {self.total_train_batches} - {self.get_metrics(trainer, pl_module)}"
            )
...
class DrivoRAgent(AbstractAgent):
    def __init__(
        ...
        lr_args: dict,
        output_dir: str,
        trajectory_sampling: TrajectorySampling = None,  # Too avoid
        train_test_split: str = "navmini",
        ...
        self.train_metric_cache_paths_synthetic = {}
        self.test_metric_cache_paths_synthetic = {}
        cache_data = False
        ...
        if not cache_data and self._checkpoint_path == "":  # only for training
            ...
            syn_idx_str = os.getenv("SYN_IDX", None)
            if syn_idx_str is not None:
                syn_indices = [int(x.strip()) for x in syn_idx_str.split(",")]
                self.train_metric_cache_paths_synthetic = {}
                for syn_i in syn_indices:
                    mc = MetricCacheLoader(
                        Path(os.getenv("NAVSIM_EXP_ROOT") + f"/train_metric_synthetic_reaction_pdm_v1.0-{syn_i}")
                    )
                    self.train_metric_cache_paths_synthetic.update(mc.metric_cache_paths)
    ...
    def get_training_callbacks(self):
        ...
        if self.progress_bar:
            ...
            return [checkpoint, progress_bar, lr_monitor]
